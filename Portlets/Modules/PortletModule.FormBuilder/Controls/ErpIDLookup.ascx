    <%@ Control Language="c#" AutoEventWireup="false" Codebehind="ErpIDLookup.ascx.cs" Inherits="Jenzabar.Portal.Framework.Web.PortletModules.FormBuilder.Controls.ErpIDLookup" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
<style type="text/css">
/*
...
<script language="javascript" type="text/javascript">
    ...
    var xmlDocStates;
    if (/MSIE (\d+\.\d+);/.test(navigator.userAgent)) {
        xmlDocStates = new ActiveXObject("Microsoft.XMLDOM");
        xmlDocStates.async = "false";
        xmlDocStates.loadXML($this.statesList);
    } else {
        var parser = new DOMParser();
        var xmlDocStates = parser.parseFromString($this.statesList, "text/xml");
    }
    var nodes = xmlDocStates.getElementsByTagName('States')[0].childNodes;
    c = 1;
    for (i = 0; i < nodes.length; i++) {
        try {
            document.getElementById("$_selState").options[c] = new Option(nodes[i].childNodes[1].childNodes[0].nodeValue, nodes[i].childNodes[0].childNodes[0].nodeValue);
            c++;
        } catch (err) {
