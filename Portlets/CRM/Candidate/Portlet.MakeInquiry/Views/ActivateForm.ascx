<%@ Control Language="c#" AutoEventWireup="True" Codebehind="ActivateForm.ascx.cs" Inherits="Jenzabar.CRM.Candidate.Web.Portlets.MakeInquiry.ActivateForm" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
<script language="javascript" type="text/javascript">
    function disableoption(control, option) {
        ...
        $(":radio").each(function () {
            ...
            var index = ra.id.indexOf(name);

            if (index > -1) {
                var sub = ra.id.substring(index, ra.id.length);
                if (sub == name) {
                    ra.disabled = true;
                }
            }
